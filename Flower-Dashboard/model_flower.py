...
class Model:
	def __init__(self, output_shape):
		...
		with tf.name_scope("conv5-512-1"):
			conv10 = tf.nn.relu(conv_layer(pooling5, 5, 512, '512-1'))
			
		with tf.name_scope("conv5-512-2"):
			conv11 = tf.nn.relu(conv_layer(conv10, 5, 512, '512-2'))
			
		with tf.name_scope("maxpool-6"):
			pooling6 = pooling(conv11)
			
		with tf.name_scope("fc-1024"):
			pooling6 = tf.reshape(pooling6, [-1, 4 * 4 * 512])
			fc1 = tf.nn.sigmoid(fully_connected(pooling6, 1024, '1024'))
