...
accept_labels = ['bicycle', 'bus', 'car', 'motorbike', 'person']
...
def bboxes_draw_on_img(img, classes, scores, bboxes, thickness=2):
    ...
    for i in range(bboxes.shape[0]):
        ...
        p2 = (int(bbox[2] * shape[0]), int(bbox[3] * shape[1]))
        cv2.rectangle(img, p1[::-1], p2[::-1], color, thickness)
        scalefactor = (((p2[0] / (shape[1] * 1.0)) + (p2[1] / (shape[0] * 1.0))) / ((shape[0] / (shape[1] * 1.0)) * 2.666666667))
        distance = ((shape[0] * shape[1]) / ((p2[0] * p2[1] * 1.0))) * scalefactor
        s = '%s / %.3fm' % (label, distance)
        p1 = (p1[0]-5, p1[1])
        cv2.putText(img, s, p1[::-1], cv2.FONT_HERSHEY_DUPLEX, 0.4, color, 2)
