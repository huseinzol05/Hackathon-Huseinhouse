...
data, output_dimension, label = utils_emotion.get_dataset()
data, data_test = train_test_split(data, test_size = 0.1)

sess = tf.InteractiveSession()

model = model_emotion.Model(output_dimension)
...
for i in xrange(settings_emotion.epoch):
    ...
    for k in xrange(0, (data.shape[0] // settings_emotion.batch_size) * settings_emotion.batch_size, settings_emotion.batch_size):
            
        emb_data = np.zeros((settings_emotion.batch_size, settings_emotion.picture_dimension, settings_emotion.picture_dimension, 1), dtype = np.float32)
        emb_data_label = np.zeros((settings_emotion.batch_size, output_dimension), dtype = np.float32)
            
        for x in xrange(settings_emotion.batch_size):
                
            image = misc.imread('data/' + data[k + x, 0])
            image = misc.imresize(image, (settings_emotion.picture_dimension, settings_emotion.picture_dimension))
    ...
    total_accuracy /= (data.shape[0] // settings_emotion.batch_size)
    total_cost /= (data.shape[0] // settings_emotion.batch_size)
    ...
    emb_data = np.zeros((data_test.shape[0], settings_emotion.picture_dimension, settings_emotion.picture_dimension, 1), dtype = np.float32)
    emb_data_label = np.zeros((data_test.shape[0], output_dimension), dtype = np.float32)
	
    for x in xrange(data_test.shape[0]):
        image = misc.imread('data/' + data_test[x, 0])
        image = misc.imresize(image, (settings_emotion.picture_dimension, settings_emotion.picture_dimension))
...
saver.save(sess, os.getcwd() + "/modelemotion.ckpt")
